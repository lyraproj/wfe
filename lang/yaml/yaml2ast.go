...
func (yp *transformer) transformMapItem(mi *yaml.MapItem, top bool) (expr parser.Expression) {
	...
	case `parallel`, `sequential`:
		...
		if top {
			if ll, ok := expr.(*parser.LiteralList); ok {
				...
				panic(eval.Error(WF_YAML_ILLEGAL_TYPE, issue.H{`path`: yp.path(), `expected`: `List`, `actual`: expr.Label()}))
			}
		} else {
	...
	default:
				...
				if tn, ok := els[ne-1].(*parser.LiteralString); ok {
					if validator.CLASSREF_EXT.MatchString(tn.StringValue()) {
						...
						if ne > 1 {
							...
							panic(eval.Error(WF_YAML_ILLEGAL_TYPE, issue.H{`path`: yp.path(), `expected`: `method name`, `actual`: name.Label()}))
						}
		...
		if expr == nil {
			if top {
				panic(eval.Error(WF_YAML_ILLEGAL_TYPE, issue.H{`path`: yp.path(), `expected`: `Name or Type`, `actual`: tk.Label()}))
			}
...
func (yp *transformer) transformEvalValue(expr parser.Expression) parser.Expression {
	...
	case *parser.LiteralList, *parser.LiteralHash:
		return expr
	default:
		panic(eval.Error(WF_YAML_ILLEGAL_TYPE, issue.H{`path`: yp.path(), `expected`: `String, List, or Hash`, `actual`: expr.Label()}))
	}
}
...
func (yp *transformer) transformGetvar(expr parser.Expression) parser.Expression {
	...
	default:
		panic(eval.Error(WF_YAML_ILLEGAL_TYPE, issue.H{`path`: yp.path(), `expected`: `String`, `actual`: expr.Label()}))
	}
}
...
func (yp *transformer) transformSetvar(expr parser.Expression) parser.Expression {
	if ll, ok := expr.(*parser.LiteralHash); ok {
		...
		for i, v := range ll.Entries() {
			...
			if n, ok := ke.Key().(*parser.LiteralString); ok {
				...
				panic(eval.Error(WF_YAML_ILLEGAL_TYPE, issue.H{`path`: yp.path(), `expected`: `String`, `actual`: ke.Key().Label()}))
			}
		}
		if len == 1 {
			return ets[0]
	...
	panic(eval.Error(WF_YAML_ILLEGAL_TYPE, issue.H{`path`: yp.path(), `expected`: `Hash`, `actual`: expr.Label()}))
}
	...
	for i, e := range assocs {
		...
		if !ok {
			if _, ok := assoc.Key().(*parser.ConcatenatedString); ok {
				...
				panic(eval.Error(WF_YAML_ILLEGAL_TYPE, issue.H{`path`: yp.path(), `expected`: `String`, `actual`: assoc.Key().Label()}))
			}
		}
		if _, ok = unique[key]; ok {
			panic(eval.Error(WF_YAML_DUPLICATE_KEY, issue.H{`path`: yp.path(), `key`: key}))
		}
		yp.popPath()
		unique[key] = true
		entries[i] = assoc
...
func (yp *transformer) transformAssoc(expr parser.Expression) *parser.KeyedEntry {
	...
	if !ok && len(ev.Entries()) == 1 {
		panic(eval.Error(WF_YAML_ILLEGAL_TYPE, issue.H{`path`: yp.path(), `expected`: `one element Hash`, `actual`: expr.Label()}))
	}
...
func (yp *transformer) resourceExpression(name *parser.QualifiedName, hash *parser.LiteralHash) parser.Expression {
	...
	for i, ev := range hash.Entries() {
		...
		if !ok {
			panic(eval.Error(WF_YAML_ILLEGAL_TYPE, issue.H{`path`: yp.path(), `expected`: `LiteralHash`, `actual`: entry.Value().Label()}))
		}
...
func (yp *transformer) attributeOperations(hash *parser.LiteralHash) []parser.Expression {
	...
	for i, ev := range entries {
		...
		if !ok {
			panic(eval.Error(WF_YAML_ILLEGAL_TYPE, issue.H{`path`: yp.path(), `expected`: `String`, `actual`: name.Label()}))
		}
		if tn == `*=>` {
	...
	for i, next := range els {
		...
		if expr == nil {
			...
			if _, ok := next.(*parser.QualifiedName); !ok {
				...
				panic(eval.Error(WF_YAML_ILLEGAL_TYPE, issue.H{`path`: yp.path(), `expected`: `method name`, `actual`: next.Label()}))
			}

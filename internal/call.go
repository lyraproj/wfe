...
func newCall(c px.Context, def serviceapi.Definition) wfe.Step {
	r := &call{}
	r.initStep(def)
	call := wfe.GetStringProperty(def, `call`)
	hclog.Default().Debug(`resolving step call`, `name`, r.name, `call`, call)
	r.ra = CreateStep(c, wfe.GetDefinition(c, px.NewTypedName(px.NsDefinition, call)))
	return r
}

func (r *call) CalledStep() wfe.Step {
	return r.ra
}
...
func (r *call) WithIndex(index int) wfe.Step {
	rc := *r // Copy by value
	rc.setIndex(index)
	return &rc
}

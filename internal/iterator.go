...
func newIterator(c px.Context, def serviceapi.Definition) wfe.Step {
	...
	style := wf.NewIterationStyle(wfe.GetStringProperty(def, `iterationStyle`))
	step := CreateStep(c, wfe.GetProperty(def, `producer`, serviceapi.DefinitionMetaType).(serviceapi.Definition))
...
func (it *iterator) Producer() wfe.Step {
	return it.Step
}
...
func resolveIteratorParameters(c px.Context, it wfe.Iterator, parameters px.OrderedMap) (px.Value, px.OrderedMap) {
...
nextVar:
	for _, v := range vs {
		...
		panic(px.Error(wfe.IterationStepWrongParameters, issue.H{`iterator`: it}))
	}
}

func assertInt(t wfe.Iterator, arg px.Value, name string) int64 {
	iv, ok := arg.(px.Integer)
	if !ok {
		panic(px.Error(wfe.IterationParameterWrongType, issue.H{
			`iterator`: t, `parameter`: name, `expected`: `Integer`, `actual`: arg.PType()}))
	}
	return iv.Int()
}

func assertRange(t wfe.Iterator, arg px.Value) (int64, int64) {
...
func assertList(t wfe.Iterator, arg px.Value) px.List {
...
func assertMap(t wfe.Iterator, arg px.Value) px.OrderedMap {
...
func iterLabel(it wfe.Iterator) string {
...
func (t *each) Run(ctx px.Context, parameters px.OrderedMap) px.OrderedMap {
	over, vars := resolveIteratorParameters(ctx, t, parameters)

...
func init() {
	loader.SmartPathFactories[wfe.LyraLinkPath] = newLyraLinkPath
	loader.SmartPathFactories[wfe.GoPluginPath] = newGoPluginPath
	loader.SmartPathFactories[wfe.PpManifestPath] = newPpManifestPath
	loader.SmartPathFactories[wfe.YamlManifestPath] = newYamlManifestPath
}
...
func instantiateGoPlugin(c px.Context, l loader.ContentProvidingLoader, _ px.TypedName, sources []string) {
...
func instantiateLyraLink(c px.Context, l loader.ContentProvidingLoader, _ px.TypedName, sources []string) {
	dl := hclog.Default()
	lf := sources[0]
	...
	if !ok {
		panic(px.Error2(issue.NewLocation(lf, 0, 0), wfe.LyraLinkNoMap, issue.NoArgs))
	}
	exe := ``
	...
	if exe == `` {
		panic(px.Error2(issue.NewLocation(lf, 0, 0), wfe.LyraLinkNoExe, issue.NoArgs))
	}
...
func instantiateYaml(c px.Context, l loader.ContentProvidingLoader, _ px.TypedName, sources []string) {
	yamlServer := wfe.GetService(c, px.NewTypedName(px.NsService, `Yaml`))
	lg := hclog.Default()
	f := sources[0]
...
func instantiatePp(c px.Context, l loader.ContentProvidingLoader, _ px.TypedName, sources []string) {
	ppServer := wfe.GetService(c, px.NewTypedName(px.NsService, `Puppet`))
	lg := hclog.Default()
	f := sources[0]
	lg.Debug("loading manifest", "file", f)
	def := ppServer.Invoke(
...
func convertToDebug(c px.Context, dc px.Value, cmd string, cmdArgs []string) (string, []string) {
	...
	if found {
		...
		cmdArgs = append([]string{`exec`, cmd, `--headless`, `--listen`, listen, `--log-dest`, `2`, `--api-version`, apiVer}, cmdArgs...)
		cmd = dlv
	}
	return cmd, cmdArgs
}
...
func loadPluginMetadata(c px.Context, dl px.DefiningLoader, cmd string, cmdArgs ...string) {
	if dc, ok := c.Get(wfe.LyraDlvConfigKey); ok {
		cmd, cmdArgs = convertToDebug(c, dc.(px.Value), cmd, cmdArgs)
	}
	serviceCmd := exec.CommandContext(c, cmd, cmdArgs...)
	svc, err := grpc.Load(serviceCmd, nil)
	if err != nil {
		panic(px.Error(wfe.FailedToLoadPlugin, issue.H{`executable`: cmd, `message`: err.Error()}))
	}
	...
	dl.SetEntry(ti, px.NewLoaderEntry(svc, nil))

	lg.Debug("loading metadata", "plugin", cmd)
	loadMetadata(c, dl, svc)
	lg.Debug("done loading metadata", "plugin", cmd)
}

func loadMetadata(c px.Context, l px.DefiningLoader, service serviceapi.Service) {
